...
function phoneService(phone, option, cb) {
  ...
  var parallel = (typeof option.parallel !== undefined && !isNaN(option.parallel)) ? option.parallel : 2; //默认两个并发
  if (option.plugins) {
    //如果使用者指定插件，则优先使用。
  ...
  //parallel 为0时使用最大并发数同时发起请求
  if (parallel === 0) {
    parallel = currentModelPlugin.length
  }
  ...
  async.parallel(listFn, function(err, results) {
    if (util.isObject(result) && result['provice']) {
      result['province'] = result['provice']
    }
    cb(errMsg, result);
  });
}
...
// support promise
function promiseApi(phone, option, cb) {
  if (util.isFunction(option)) {
    cb = option;
    option = {};
  }
  return new Promise(function(resolve, reject) {
    phoneService(phone, option, function(err, data) {
      if (err) {
        return reject(err)
      }
      resolve(data)
